...
////// GLOBAL TEMPLATE PARTS ------> START
    ...
    /**
     * A filter for body_class (Used for: sticky menu, etc.)
     */
    public static function body_class( $classes ) {
      ...
      $topbar_sticky    = Plethora_Theme::option( THEMEOPTION_PREFIX .'header-sticky-topbar', 0);

      $classes[] = $header_sticky ? 'sticky_header' : '';
      $classes[] = $header_bcg_trans ? 'transparent_header' : '';
      $classes[] = $topbar_sticky ? 'sticky_topbar' : '';
    ...
    function global_title() {
     ...
     if (  !( class_exists( 'woocommerce') && is_shop() )
            && ! is_singular( array('post', 'profile' ) )
            && ! is_404()
            && Plethora_Theme::content_has_titles()
        ) {
     ...
     */
    public static function global_title_no_padding_to_content( $class ) {
    ...
    public static function footer_infobar(){
      ...
      if ( $footer_infobar ) {
        ...
        $atts['copyright']       = do_shortcode( $atts['copyright'] );
        $atts['credits']         = do_shortcode( $atts['credits'] );
        set_query_var( 'atts', $atts );
        ...
        <div class="new_comment">
            ...
            $new_comment_args = array(
              'fields'               => apply_filters( 'comment_form_default_fields', array(
                                          ...
                                          'cookies' => '<div class="row"><div class="col-sm-12 col-md-8 comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' /> ' . '<label for="wp-comment-cookies-consent">' . esc_html__( 'Save my name, email, and website in this browser for the next time I comment.', 'healthflex' ) . '</label></div></div>',
                                        )),
