...
abstract class Plethora {
	...
	public function load_framework() {
		...
		// Migrating to PFL v2
		require_once( PLE_CORE_HELPERS_DIR .'/plethora-helper.php' );
...
// PUBLIC REUSABLE STATIC METHODS ----> START
	...
	*/
	...
	public static function get_non_plethora_post_types( $args = array() ) {
		...
		// Merge user given arguments with default
		$args = wp_parse_args( $args, $default_args);
		unset( $args['plethora_only'] ); // just in case
		$args_plethora_only = $args;
		$args_plethora_only['plethora_only'] = true;

		$all_pts = self::get_supported_post_types( $args );
		$plethora_only_pts = self::get_supported_post_types( $args_plethora_only );
		$non_plethora_pts = array_diff_key( $all_pts, $plethora_only_pts );
		return $non_plethora_pts;
	}
	...
	*/
	...
	/**
		* Locates template files within the given path and returns several administrating information.
		*
		* Since its main purpose is to return template files information and supported options, it will respect child theme templates/overrides if 'child_templates' argument is set to true.
		* The $path parameter must contain a theme root folder path ( ie. '/templates/shortcodes'). It will return all meta values included in file, in a manner similar to
		* the core style.css file.
		*
		* Arguments:
		* 	array(
		*		'filter_prefix'    => '',  		Prefix string filter to match the beggining of the filename
		*		'filter_suffix'    => '',  		Prefix string filter to match the end of the filename
		*		'meta_data'        => false,    If true, it will return meta data information
		*		'supported_params' => array(), 	An array of arrays, such as array( 'param' => array( 'option_to_search1', 'option_to_search2' ) )
		*		'child_templates'  => true, 	If true, it will respect the parent/child theme hierarchy
		*		'return'           => 'all' 	'all' will return full info, while 'basic' will return only a simple key => title array for use within a select option
		* 	)
		*
		* Example return for shortcode-button.php and filter_prefix set to 'shortcode-':
		* 	array(
		*		'slug'             => 'shortcode-button',
		* 		'slug_filtered'    => 'shortcode-button',
		* 		'file_name'        => 'button.php',
		* 		'file_path'        => '\httdocs\wp-content\themes\mytheme\templates\shortcodes\shortcode-button.php',
		*		'template_path'    => 'templates/shortcodes/button.php',
		* 		'title'            => 'Title given in file meta section if exists OR the filename capitalized and spaces instead of dashes/hyphens',
		* 		'params_supported' => array( 'supported_field1', 'supported_field2' ),
		* 		{custom_key}       => {custom value given in file}
		* 	)
		*
		* You may add as many meta key/values needed on each file, required that they will be given on separate lines ( similar to the theme's style.css )
		*
		* @param string $path The directory path, relative to theme root folder ( ie. 'templates/shortcodes' )
		* @param array $args An array with arguments ( check description )
		* @return array It will return array with template information ( check description )
		* @api Features API
		* @since 2.0.1 New method added.
	*/
	public static function get_path_templates( $rel_dir_path, $args = array()  ) {
		...
		$args_defaults = array(
			...
			'child_templates'  => true,
			'return'           => 'all'
		);
		$args = wp_parse_args( $args, $args_defaults );
		extract( $args );
		$parent_dir = file_exists( PLE_THEME_DIR . '/' . $rel_dir_path ) ? PLE_THEME_DIR . '/' . $rel_dir_path : false;
		$child_dir  = $child_templates && is_child_theme() && file_exists( PLE_CHILD_DIR . '/' . $rel_dir_path ) ? PLE_CHILD_DIR . '/' . $rel_dir_path : false;
		if ( is_admin() && $parent_dir ) {

			// get parent files
			$files_found = glob( $parent_dir . '/' . $filter_prefix .'*'. $filter_suffix .'.php' );

			// merge with child files, if any
			$child_files_found  = array();
			if ( $child_dir ) {

				$child_files_found = glob( $child_dir . '/' . $filter_prefix .'*'. $filter_suffix .'.php' );
				$files_found       = array_merge( $files_found, $child_files_found );
			}
			foreach ( $files_found as $filepath ) {

				$filename_with_ext                         = basename( $filepath );
				$filename_no_ext                           = str_replace( '.php', '', $filename_with_ext );
				$slug_filtered                             = substr_replace( $filename_no_ext, '', 0, strlen( $filter_prefix ) );
				$slug_filtered                             = substr_replace( $slug_filtered, '', strlen( $slug_filtered ) - strlen( $filter_suffix ), strlen( $filter_suffix ) );
				$path_templates[$rel_dir_path . '/'. $filename_no_ext] = array(
					'slug'          => $filename_no_ext,
					'slug_filtered' => $slug_filtered,
					'file_name'     => $filename_with_ext,
					'file_path'     => $filepath,
					'template_path' => $rel_dir_path . '/'. $filename_no_ext,
					'title'         => Plethora_Helper::slug_to_title( $filename_with_ext, 'all', '.php' ),
				);

				if ( $supported_params ) { // find supported parameters and append them to the return value

					$path_templates[$rel_dir_path . '/'. $filename_no_ext]['supported_params'] = self::get_supported_template_params( $filepath, $supported_params );
				}

				if ( $meta_data ) { // find meta key/values and append them to the return value

					$metas = self::get_file_meta( $filepath );
					foreach ( $metas as $meta_key => $meta_val ) {
						if ( $meta_key && ! in_array( $meta_key, array( 'slug', 'file_name', 'file_path', 'template_path' ) ) ) {

							// if 'default' meta_key exists and is true, then add this information to the title
							if ( $meta_key === 'default' && $meta_val ) {

								$path_templates[$rel_dir_path . '/'. $filename_no_ext]['title'] = $path_templates[$rel_dir_path . '/'. $filename_no_ext]['title'] .' ( default )';
							}
							$path_templates[$rel_dir_path . '/'. $filename_no_ext][$meta_key] = $meta_val;
						}
					}
				}
			}
	...
	/**
	* Searches a file for meta data and returns them in key/value array
	*
	* It will return all key: value matches included within the first comment starting with /*! and ends with inverse string
	*
	* @param string $file_dir_path The full directory path ( ie. '/httpdocs/wp-content/theme/mytheme/templates/shortcodes/test.php' )
	* @return array It will return array with all meta data in key/value array
	* @api Features API
	* @since 2.0.2 New method added.
	*/
	public static function get_file_meta( $file_dir_path, $meta_key = 'all' ) {

		$file_meta = $meta_key === 'all' ? array() : '';
		if ( file_exists( $file_dir_path ) ) {

			$file_contents = Plethora_Helper::get_file_contents( $file_dir_path );
			$reg_pattern = '/\/\*\!(.*?)\!\*\//s';
			preg_match_all($reg_pattern, $file_contents, $meta_raw, PREG_SET_ORDER, 0);
			if ( !empty( $meta_raw[0][1] ) ) {

				$meta_reg_pattern = '/(?:[\n]{0,1})(\w+)(?:\ *:\ *)([^\n]*)(?:[\n]{0,1})/m';
				preg_match_all($meta_reg_pattern, $meta_raw[0][1], $metas, PREG_SET_ORDER, 0);
				foreach ( $metas as $meta ) {
					$key = !empty( $meta[1] ) ? sanitize_key( $meta[1] ) : false;
					$val = !empty( $meta[2] ) ? trim( esc_html( $meta[2] ) ) : '';

					if ( $key ) {

						// if $meta_key is not 'all', then compare with the current key and return accordingly
						if ( $meta_key !== 'all' && $meta_key === $key ) {

							return $val;

						} elseif (  $meta_key === 'all' ) {

							// if 'default' key exists, make it a legit boolean value
							if ( $key === 'default' ) {

								$val = in_array( $val, array( '1', 'true' ) ) ? true : false;
							}
	...
	*/
	...
	public static function get_file_hierarchy_info( $rel_file_path, $return = '' ) {

		$filename = basename( $rel_file_path );
		$search_locations['plugin'] = array(
			'position_title' => esc_html__( 'Framework ( Plugin )', 'plethora-framework' ),
			'position_desc' => esc_html__( 'File loaded via Plethora Features Library  ( Plugin )', 'plethora-framework' ),
			'rel_path'       => $rel_file_path,
			'dir_path'       => PLE_FLIB_DIR . '/' . $rel_file_path,
			'url_path'       => PLE_FLIB_URI . '/' . $rel_file_path
		);
		$search_locations['core'] = array(
			'position_title' => esc_html__( 'Framework ( Core )', 'plethora-framework' ),
			'rel_path'       => $rel_file_path,
			'dir_path'       => PLE_CORE_DIR . '/' . $rel_file_path,
			'url_path'       => PLE_CORE_URI . '/' . $rel_file_path
		);
		$search_locations['parent_theme'] = array(
			'position_title' => esc_html__( 'Parent theme', 'plethora-framework' ),
			'rel_path'       => $rel_file_path,
			'dir_path'       => PLE_THEME_DIR . '/' . $rel_file_path,
			'url_path'       => PLE_THEME_URI . '/' . $rel_file_path
		);
		if ( is_child_theme() ) {

			$search_locations['child_theme'] = array(
				'position_title' => esc_html__( 'Child theme', 'plethora-framework' ),
				'rel_path'       => $rel_file_path,
				'dir_path'       => PLE_CHILD_DIR . '/' . $rel_file_path,
				'url_path'       => PLE_CHILD_URI . '/' . $rel_file_path
			);
		}

		$hierarchy_info = array();
		foreach ( $search_locations as $pos => $loc ) {

			if ( file_exists( $loc['dir_path'] ) ) {

				$hierarchy_info['filename']       = $filename;
				$hierarchy_info['position']       = $pos;
				$hierarchy_info['position_title'] = $loc['position_title'];
				$hierarchy_info['rel_path']       = $loc['rel_path'];
				$hierarchy_info['dir_path']       = $loc['dir_path'];
				$hierarchy_info['url_path']       = $loc['url_path'];
				$hierarchy_info['sequence'][$pos] = $loc['dir_path'];
			}
		}
		if ( !empty( $return ) ) {

			if ( isset( $hierarchy_info[$return] ) ) {

				return $hierarchy_info[$return];
			}

			return $return === 'sequence' ? array() : '';
