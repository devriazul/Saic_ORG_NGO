	...
	class Plethora_Module_Icons {
		...
		public function __construct() {
			...
			if ( $assets_directory ) {
    			...
    			add_filter( 'vc_map_get_attributes', function( $atts, $tag ) { // FIX THE FUCKING VC BUG!

   					if ( $tag == 'vc_icon' ) {
   						if ( $atts['type'] === 'webfont_medical_icons' ) {

   							$atts['icon_webfont_medical_icons'] = '';
   						}
    				}
    				return $atts;
    			}, 10, 2 );
...
// Font Libraries Management Methods ---> START
		...
		*/
		...
		private function remove_junk_font_files() {
			...
			if ( !is_array( $existing_files ) ) return false;
...
// Admin User Interaction Methods ---> START
		...
		public function get_preset_libraries_desc() {
			...
			$subtitle_text .= '<li><a href="'. esc_url( 'http://fontawesome.io/icons/' ) .'" target="_blank">Font Awesome</a></li>';
			...
			$subtitle_text .= '<li><a href="'. esc_url( 'https://erikflowers.github.io/weather-icons/' ) .'" target="_blank">Weather Icons</a></li>';
		...
		public static function vc_iconpicker_plethora_icons( $icons ) {
			...
			if ( $current_filter === 'vc_iconpicker-type-plethora' ) {
				...
				$plethora_icons = self::get_options_array( array( 'use_in' => 'vc', 'grouped_results' => true ) );
 					...
 					ob_start(); print_r( $icons ); error_log( ob_get_clean() );
