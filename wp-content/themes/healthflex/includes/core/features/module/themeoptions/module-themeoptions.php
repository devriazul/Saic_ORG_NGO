...
//// LESS CONFIGURATION BEGINS
		public function less_variables( $vars ) {
		...
		// MISC
			...
			$vars['wp-woo-enabled'] = ( class_exists( 'WooCommerce' ) ) ? "woocommerce" : "blank";
...
//// STATIC HELPER METHODS START
		...
		* of the given options index and configuration methods
		* @return array
		*/
		public static function get_themeoptions_fields( $feature_obj, $index_method = 'options_index', $config_method = 'options_config' ) {

			$fields = array();
			if ( ! is_object( $feature_obj ) && ( method_exists( $feature_obj, $index_method ) && method_exists( $feature_obj, $config_method ) ) ) {

				return $fields;
			}
			// setup theme options according to configuration
			$opts        = call_user_func( array( $feature_obj, $index_method ) );
			$opts_config = call_user_func( array( $feature_obj, $config_method ) );
		...
		* @return array
		*/
		public static function get_metabox_fields( $feature_obj, $index_method = 'options_index', $config_method = 'options_config' ) {
		...
		*/
		public static function get_option_default_value( $feature_obj, $option_id, $index_method = 'options_index', $config_method = 'options_config' ) {

			$val = null;
			if ( is_object( $feature_obj ) && ( method_exists( $feature_obj, $index_method ) && method_exists( $feature_obj, $config_method ) ) ) {

				$options = self::get_themeoptions_fields( $feature_obj, $index_method, $config_method ); // all default values
