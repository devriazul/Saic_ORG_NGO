	    ...
	    */
		...
		public function register_thirdparty_libs() {
		    ...
		    # JS LIBRARIES  
		      ...
		      wp_register_script( 'counter-up', $this->libdir_js .'counter-up/jquery.counterup'.$this->min_suffix.'.js', array( 'jquery', 'waypoints' ), '1.0', true  );
...
// START: CUSTOM SCRIPT/STYLE OUTPUT METHODS
	    ...
	    /**
	     * Adds custom Body Class (Theme Options > General > Misc)
	     *
	     */
		public function add_custom_body_class($classes){

			$custom_body_classes = Plethora_Theme::option(METAOPTION_PREFIX .'custombodyclass');
			if ( !empty( $custom_body_classes ) ) { 
				$classes[] = $custom_body_classes;
			} 
			return $classes; 
