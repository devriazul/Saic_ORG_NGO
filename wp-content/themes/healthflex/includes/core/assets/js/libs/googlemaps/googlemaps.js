  ...
  function mapsInit() {
    ...
    themeConfig["GOOGLE_MAPS"].maps.forEach(function(map){
      ...
      if ( mapElement !== null ){
        ...
        // ARE MARKERS ENABLED?
        if ( _p.checkBool(map.marker) ){

          // IS ANIMATED MARKER ENABLED?
          if ( !map.pins && _p.checkBool(map.animatedMarker) ) {

            var overlay = new animatedPin( myLatlng, gmap, 
              { 
          ...
          // DEFAULT MARKER(S)
            ...
            google.maps.event.addListener( marker, 'click', (function(marker, i) {
              ...
              return function() {
                infowindow.setContent( map['desc'] );
                infowindow.open( gmap, marker );
              }

            })(marker));

            // ARE MULTIPLE MARKERS ENABLED?
            if ( map.pins ){
              ...
              for ( var i = 0; i < pinsLength; i++ ) {  
                ...
                (function(pin){
                  ...
                  var point = new google.maps.Marker({
                    position : new google.maps.LatLng( pin.pin_lat, pin.pin_lon ),
                    map      : gmap
                  });

                  google.maps.event.addListener(point, 'click', (function(marker, i) {

                    return function() {
                      infowindow.setContent( pin.pin_desc );
                      infowindow.open( gmap, marker );
                    }
