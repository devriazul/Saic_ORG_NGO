	...
	class Redux_WordPress_Data extends Redux_Class {
		...
		private function get_data( $type, $args, $current_value ) {
			...
			switch ( $type ) {
				...
				case 'terms':
				case 'term':
					// PLEFIX: Made it compatible with post WP 4.5 changes to get_terms() - Plethora
					$args = isset( $args['taxonomies'] ) ? array( 'taxonomy' => array_values($args['taxonomies']) ) : array( 'taxonomy' => array_values($args) ) ;
					// PLEFIXME ENDED
