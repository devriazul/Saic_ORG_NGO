...
//// AJAX ONLY METHODS START
	...
	private function get_import_mask( $for_import_table = false ) {
	  ...
	  if ( $for_import_table ) {
	  	...
	  	foreach ( $this->widgets as $sidebar => $widgets ) {
	  		...
	  		$sidebar_counter++;
	  		$widget_counter = $widget_counter + count( get_object_vars( $widgets ) );
	  	}
	  }
	  $import_mask['widgets'] =  array(
									'title'             => esc_html__( 'Widgets', 'plethora-framework' ),
									'counter'           => $widget_counter,
									'in_queue_notice'   => sprintf( esc_html__('%s sidebars with total %s widgets in queue', 'plethora-framework' ), $sidebar_counter, $widget_counter ),
	  ...
	  if ( $for_import_table ) {
	  	...
	  	foreach ( $this->db_tables as $table => $records ) {

	  		$table_counter++;
	  		$record_counter = $record_counter + count( $records );
	  	}
	  }
	  $import_mask['db_tables'] =  array(
									'title'             => esc_html__( 'Database Tables', 'plethora-framework' ),
									'counter'           => $table_counter,
									'in_queue_notice'   => sprintf( esc_html__('%s tables with total %s records in queue', 'plethora-framework' ), $sidebar_counter, $record_counter ),
									'beforeSend_notice' => esc_html__( 'Importing database tables...', 'plethora-framework' ),
									'success_notice'    => esc_html__( 'All tables imported!', 'plethora-framework' ),
									'error_notice'      => $error_notice,
									'response_method'   => 'db_tables'
								  );
	  ...
	  if ( $for_import_table ) {
		...
		if ( empty( $this->demo->db_tables ) ) { unset( $import_mask['db_tables'] ); }
	...
	public function set_demo() {
	  ...
	  if ( !empty( $this->demo ) ) { // Additional data setup
		...
		$this->db_tables      = !empty( $demo->db_tables ) ? $demo->db_tables : array() ;
	  ...
	  if ( $this->verify_nonce() && $this->set_demo() ) { // verify nonce and selected demo data first!
		...
		if ( !empty( $response ) ) {
		  ...
		  switch ( $response ) {
			...
			case 'db_tables':
			  echo $this->import_db_tables();     // ajax response, no late escape applied here!
			  break;
			...
			case 'finish':
			  ...
			  do_action( 'plethora_demo_import_finished' );
	...
	public function import_terms() {
	  ...
	  foreach ( $this->terms as $term ) {
		...
		if ( ! is_wp_error( $new_term ) ) {
		  ...
		  $this->errors['terms']['user'][] = esc_html__( 'Failed to import post tern: ', 'plethora-framework' ) . esc_html($term->name);
	...
	function fetch_remote_file( $url, $post ) {
	  ...
	  if ( $headers['response']['code'] != '200' ) {
		@unlink( $upload['file'] );
		return new WP_Error( 'import_file_error', sprintf( esc_html__('Remote server returned error response %1$d %2$s', 'plethora-framework'), esc_html($headers->get_error_message()), $headers->get_error_code() ) );
	  }
	...
	public function import_db_tables() {

        global $wpdb;
		$raw_data = $this->db_tables;
		foreach ( $raw_data as $unprefixed_table => $records ) {

			if ( $this->table_exists( $unprefixed_table ) ) {

				foreach ( $records as $record ) {

					$wpdb->insert( $this->get_prefixed_table( $unprefixed_table ), get_object_vars( $record ) );
				}
			}
		}
	}
	...
	public function set_menu_locations() {
	  ...
	  foreach ( $registered_nav_menus as $regmenu_key => $regmenu_name ) {
		...
		if ( isset( $this->menu_locations->$regmenu_key )  ) {
...
//// UI METHODS START
	...
	public function ui_header() {
	  ...
	  $header .= '<p>'. sprintf( esc_html__('Preview and select a demo to import. We always recommend importing only one demo at a single installation. Before trying to import the demo of your choice, make sure that you have installed / activated all required plugins.'), '<strong style="color:darkred">', '</strong>' ) .'</p>';
	  return $header;
	}
...
//// HELPERS START
   ...
   /**
   * Returns given plugin related DB table contents
   * @return array
   */
    public function table_exists( $unprefixed_table ) {

        $all_plugin_tables = $this->get_plugin_tables();
        if ( in_array( $unprefixed_table, $all_plugin_tables ) ) {
    ...
    public function get_all_tables() {
        ...
        $results = $wpdb->get_results( "SHOW TABLES LIKE '$blog_prefix%'" );
        $all_tables = array();
        foreach( $results as $index => $value ) {
            foreach( $value as $prefixed_table ) {

                $all_tables[] = $this->get_unprefixed_table( $prefixed_table );
            }
        }
        return $all_tables;
    }
   ...
   /**
   * Returns all WP core related DB tables
   * @return array
   */
    public function get_wp_tables() {

        // Get first all WP related tables
        global $wpdb;
        $results = $wpdb->tables( 'blog', true, get_current_blog_id() );
        $wp_tables = array();
        foreach( $results as $prefixed_table ) {

          $wp_tables[] = $this->get_unprefixed_table( $prefixed_table );
        }
        return $wp_tables;
    }

   /**
   * Returns all plugin related DB tables
   * @return array
   */
    public function get_plugin_tables() {

        // Get first all WP related tables
        $all_tables    = $this->get_all_tables();
        $wp_tables     = $this->get_wp_tables();
        $tables        = array_diff( $all_tables , $wp_tables );
        $plugin_tables = array();
        foreach ( $tables as $table ) {
          $plugin_tables[$table] = $table;
        }
   ...
   * @return array
   */
    public function get_prefixed_table( $unprefixed_table ) {

        global $wpdb;
        $prefixed_table = $wpdb->get_blog_prefix() . $unprefixed_table;
   ...
   * @return array
   */
    public function get_unprefixed_table( $prefixed_table ) {
        global $wpdb;
        $prefix = $wpdb->get_blog_prefix();
        $prefix_len = strlen( $prefix );
        $table_len = strlen( $prefixed_table ) - $prefix_len;
        $unprefixed_table = mb_substr($prefixed_table, $prefix_len, $table_len );
        return $unprefixed_table;
    }
