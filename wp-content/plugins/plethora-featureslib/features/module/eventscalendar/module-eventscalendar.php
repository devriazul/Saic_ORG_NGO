...
if ( !class_exists('Plethora_Module_Eventscalendar') ) {
	...
	class Plethora_Module_Eventscalendar {
		...
		public function __construct() {
			...
			if ( class_exists( 'Tribe__Events__Main' ) ) {
				...
				# Admin: theme Options & metaboxes
					...
					// IMPORTANT FIX. Events calendar dequeues jquery-ui-css instances, a file that is crucial for our panels...so we have to re-enqueue this
					add_action( 'admin_enqueue_scripts', array( $this, 'admin_assets_enqueue_fix' ), 999 );

				# Front: Containers Setup ( hook on 'get_header' please )
				# Front: Templates Setup ( hook on 'get_header' please )
				# Front: Catalog Setup

					add_action( 'wp', array($this, 'archive_intro_text_hook') );
				...
				# Front: Other configuration
					...
					Plethora_Theme::add_admin_notice( 'events_calendar', array(
						...
						'notice'       => array(
																sprintf( esc_html__( 'Have some good news for you, as %s provides special support for the Events Calendar plugin.', 'plethora-framework' ), THEME_DISPLAYNAME ),
		...
		public function admin_assets_enqueue_fix() {
			...
			if ( ! $this->is_tribe_admin_page() ) {
				...
				// VERY IMPORTANT. If jquery-ui is not loaded, then our button set options look messy
		...
		public function archive_intro_text_hook() {

			$intro_text_action = Plethora_Theme::option( METAOPTION_PREFIX.'archive'.$this->post_type.'-introtext-action', 'tribe_events_before_template' );
			if ( $intro_text_action === 'tribe_events_inside_before_loop' && function_exists( 'tribe_is_month' ) && tribe_is_month() ) {

				$intro_text_action = 'tribe_events_bar_after_template';
			}
			add_action( $intro_text_action, array($this, 'archive_intro_text') );
		}


		// Set align options for archive ( if exist for this page )
		public function archive_intro_text() {

			$intro_text = Plethora_Theme::option( METAOPTION_PREFIX .'archive'.$this->post_type.'-introtext', '' );
			echo '<div class="tribe-clear"></div>';
		...
		*/
		public function archive_options() {
			...
			$archive_options['intro-text'] = array(
							...
							'type'    => 'editor',
							'title'   => esc_html__('Additional Text', 'plethora-framework'),
							'desc'    => esc_html__('Display additional content to be displayed in the calendar. Use next option to define its display position.', 'plethora-framework'),
							'args'   => array(
										'teeny'            => false,
										'textarea_rows'    => 7
							)
			);
			$archive_options['intro-text-action'] = array(
							'id'      => METAOPTION_PREFIX.'archive'.$this->post_type.'-introtext-action',
							'type'    => 'button_set',
							'title'   => esc_html__('Additional Text Display Position', 'plethora-framework'),
							'options' => array(
								'tribe_events_before_template'    => esc_html__( 'Before events title', 'plethora-framework'),
								'tribe_events_after_the_title'    => esc_html__( 'After events title', 'plethora-framework'),
								'tribe_events_inside_before_loop' => esc_html__( 'Before events', 'plethora-framework'),
								'tribe_events_after_template'     => esc_html__( 'After events', 'plethora-framework'),
							)
			);
