...
if ( !class_exists('Plethora_Module_Vc') ) {
  ...
  class Plethora_Module_Vc {
            ...
            if ( $this->update_source === 'tgm' ) {
              ...
              # BASIC CONFIGURATION
                ...
                if ( is_admin() ) {
                  ...
                  $this->add_shortcode_param( 'ple_script_loader', array( $this, 'vc_script_loader'), '');// Add custom JS file
       ...
       * @since 1.0
       */
      public function add_shortcode_param( $name, $callback_function, $js = '') {
      ...
      public function vc_script_loader( $settings ){

        $exec = ( !isset($settings['std']) )? $settings['value'] : $settings['std'];
        $script = '<script>' . $exec .';</script>';
        return $script;
      }
      ...
      public function theme_options_tab( $sections ) {
        ...
        $sections[] = array(
          'subsection' => true,
          'title'      => esc_html__('WPBakery Page Builder', 'plethora-framework'),
          'heading'    => esc_html__('WPBAKERY PAGE BUILDER', 'plethora-framework'),
          'fields'     => array(
