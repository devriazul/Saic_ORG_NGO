		...
		public function get_post_type_options( $type = 'names' ) {
			...
			if ( $type === 'names' ) {
				...
				$return = array(
					...
					'rewrite' 			  	=> array( 
													'slug'			=> sprintf( esc_html_x( '%s', 'Rewrite slug for room post type', 'plethora-framework'), Plethora_Theme::option( THEMEOPTION_PREFIX .$this->post_type .'-urlrewrite', $this->post_type ) ) , // Used as pretty permalink text (i.e. /tag/) - defaults to $taxonomy (taxonomy's name slug) 
		...
		*/
		public function single_options() {
			...
			$single_options['mediadisplay-type'] = array(
						...
						'required' => array(
										array( METAOPTION_PREFIX .$this->post_type .'-mediadisplay', 'equals', array( '1' ) ),
									  )  
						);
			...
			$single_options['amenities-single-view'] = array(
						...
						'required' => array( 
										array( METAOPTION_PREFIX .$this->post_type .'-amenities-status', 'equals', '1' )
									  ),
					  );
			...
			$single_options['persons-text']  = array(
				...
				'required'     => array(
					array( METAOPTION_PREFIX . $this->post_type .'-booking-persons', 'equals', array( '1' ) ),
				)
			);
			...
			$single_options['urlrewrite'] = array(
						...
						'desc'             => sprintf( 
							...
							esc_html__('Specify a custom permalink for %1$s post type ( i.e.: http://yoursite.com/%1$s/sample-%1$s).', 'plethora-framework')

							. '<br/><span style="color:red;">' .

							esc_html__('NOTICE: Updating this will probably result in a 404 page error in every %1$s post. This can be easily fixed with a simple click on "Save Changes" button, on the "Settings > Permalinks" screen', 'plethora-framework')
