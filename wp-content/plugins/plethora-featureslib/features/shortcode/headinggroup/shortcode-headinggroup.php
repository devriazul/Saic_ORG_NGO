  ...
  class Plethora_Shortcode_Headinggroup extends Plethora_Shortcode { 
      ...
      */
      public function params_index() {
        ...
        $params_index['subtitle_color'] = array( 
              "param_name"    => "subtitle_color",                                  
              "type"          => "colorpicker",                                        
              "holder"        => "h4",                                               
              "class"         => "plethora_vc_title",        
              "value"         => "",                                         
              "heading"       => esc_html__("Heading Subtitle Color", 'plethora-framework'),      
              "description"   => esc_html__("Set the heading subtitle color", 'plethora-framework'),       
              "admin_label"   => false,                                              
        );
        ...
        $svgs = Plethora_Theme::get_path_templates( 'templates/shortcodes', array( 'filter_prefix' => 'divider' ));
        $svgDividerTpls = array();
        foreach ( $svgs as $value ) { 
          $svgDividerTpls[ $value['title'] ] = $value['template_path']; 
        }
        $params_index['divider_template'] = array( 
              "param_name"    => "divider_template",
              "type"          => "dropdown",
              'edit_field_class' => 'vc_col-sm-6 vc_column',
              "heading"       => esc_html__('Divider Template', 'plethora-framework'),
              "value"         => $svgDividerTpls,
              "description"   => esc_html__('Choose divider template for different styling.', 'plethora-framework'),       
        );
      ...
      * @return string
      */
        public function get_svg_divider( $divider_status = false, $divider_template = '' ) {
      ...
      */
      public function content( $atts, $content = null ) {
        ...
        // Avoir, Xenia:
        $divider = ( isset( $divider ) ? isset( $divider_template ) ? $this->get_svg_divider( $divider, $divider_template ) : $this->get_svg_divider( $divider ) : '' );

        $shortcode_atts = array(
                'title'             => ( ( isset( $content ) ) ? $content : '' ),
                'subtitle_position' => ( ( isset( $subtitle_position ) ) ? "subtitle_" . $subtitle_position : '' ),
                'subtitle'          => ( ( isset( $subtitle ) ) ? $subtitle : '' ),
                'subtitle_color'     => ( ( isset( $subtitle_color ) ) ? $subtitle_color : '' ),
