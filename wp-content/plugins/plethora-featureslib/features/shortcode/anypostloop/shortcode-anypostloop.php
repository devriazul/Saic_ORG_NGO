	...
	class Plethora_Shortcode_Anypostloop extends Plethora_Shortcode {
		...
		*/
		public function params_config() {
			...
			$params_config[] = array( 'id' => 'items_media_size', 				'default' => 'large',					'field_size' => '6', );
			...
			/*
			!IMPORTANT IMPORTANT!
			When using a shortcode loop that displays post containing other shortcode loops on its editor content,
			an endless PHP loop will be created, that causes a fatal PHP out of memory error. Setting this default
			value to ZERO ( false ) eliminates the error...nevertheless, we should look into this in further
			*/
			$params_config[] = array( 'id' => 'items_editorcontent', 			'default' => '0', 						'field_size' => '6', );
			...
			$params_config[] = array( 'id' => 'nav_text_next',					'default' => 'Next',					'field_size' => '6', );
			$params_config[] = array( 'id' => 'nav_text_prev',					'default' => 'Prev',					'field_size' => '6', );
		...
		*/
		public function get_supported_template_params( $template ) {
			...
			$field_to_values_mapping = array(
				...
				'items_commentscount'           => array( 'item_comments_number', 'item_comments_link', 'item_comments_text' ),
		...
		*/
		 ...
		 */
	   	public function params_index() {
			...
			$params['items_media_size'] = array(
				'param_name'       => 'items_media_size',
				'type'             => 'dropdown',
				'group'            => esc_html__('Items Styling', 'plethora-framework'),
				'heading'          => esc_html__('Featured Image Size', 'plethora-framework'),
			...
			$params['items_link_behavior'] = array(
				...
				'description'      => esc_html__('Select secondary taxonomy to be displayed', 'plethora-framework'),
				'value'            => array(
					sprintf( esc_html__('Link to %1$s page', 'plethora-framework'), Plethora_Theme::mb_strtolower( $this->post_type->labels->singular_name ) )                 => 'normal',
					sprintf( esc_html__('Link to %1$s page, in new tab', 'plethora-framework'), Plethora_Theme::mb_strtolower( $this->post_type->labels->singular_name ) )     => 'blank',
					sprintf( esc_html__('Link to %1$s page, in ajax window', 'plethora-framework'), Plethora_Theme::mb_strtolower( $this->post_type->labels->singular_name ) ) => 'ajax',
					sprintf( esc_html__('Do not link', 'plethora-framework'), Plethora_Theme::mb_strtolower( $this->post_type->labels->singular_name ) )                       => 'no_link',
				),
				'dependency'       => array(
					'element' => 'items_template',
					'value'   => $this->get_templates_dependency( array(
			...
			$params['nav'] = array(
				...
				"type"             => "dropdown",
				'group'            => esc_html__('Slider Options', 'plethora-framework'),
				"heading"          => esc_html__("Navigation Buttons", 'plethora-framework'),
				"value"            => array( esc_html__("No", 'plethora-framework') => 'false', esc_html__("Yes", 'plethora-framework') => 'true' ),
				'dependency'       => array(
					'element' => 'items_template',
					'value'   => $this->get_templates_dependency( array(
			...
			$params['nav_text_prev'] = array(
				"param_name"       => "nav_text_prev",
				"type"             => "textfield",
				'group'            => esc_html__('Slider Options', 'plethora-framework'),
				"heading"          => esc_html__("Navigation Button Text: Previous", 'plethora-framework'),
				'dependency'       => array(
					'element' => 'items_template',
					'value'   => $this->get_templates_dependency( array(
						'display_type' => array( 'slider' ),
			...
			$params['nav_text_next'] = array(
				"param_name"       => "nav_text_next",
				"type"             => "textfield",
				'group'            => esc_html__('Slider Options', 'plethora-framework'),
				"heading"          => esc_html__("Navigation Button Text: Next", 'plethora-framework'),
				'dependency'       => array(
					'element' => 'items_template',
					'value'   => $this->get_templates_dependency( array(
						'display_type' => array( 'slider' ),
		...
		*/
		public function search_data_posts( $user_val ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( Plethora_Theme::mb_stripos( $post->post_title, $user_val ) !== false || Plethora_Theme::mb_stripos( $post->post_name, $user_val ) !== false || Plethora_Theme::mb_stripos( $this->post_type->label, $user_val ) !== false || Plethora_Theme::mb_stripos( $this->post_type->name, $user_val ) !== false ) {
					$values[] = array(
			...
			foreach ( $post_taxonomies as $tax_slug => $tax_obj ) {
				...
				if ( ! is_wp_error( $post_taxonomy_terms ) ) {
					...
					foreach ( $post_taxonomy_terms as $term  ) {
						// note: stripos MUST be checked against false with the identical comparison operator !==
						if ( Plethora_Theme::mb_stripos( $term->name, $user_val ) !== false || Plethora_Theme::mb_stripos( $term->slug, $user_val ) !== false || Plethora_Theme::mb_stripos( $tax_obj->label, $user_val ) !== false ) {
		...
		*/
		...
		public function set_scripts( $atts ) {
			...
			if ( in_array( $this->get_template_file_type( $atts['items_template'] ), array( 'slider' ) ) ) {
				...
				$slider['post_type']          = $data;	//=> 'room','testimonial','post',etc.
				...
				$slider['dots']               = $dots;
				$slider['nav']                = $nav;
				$slider['nav_text_prev']      = $nav_text_prev;
				$slider['nav_text_next']      = $nav_text_next;
				$slider['rtl']                = $rtl;
		...
		public function prepare_atts_items_for_main_template( $atts ) {
			...
			if ( $post_query->have_posts() ) {
				...
				while ( $post_query->have_posts() ) : $post_query->the_post();
					...
					$media                            =  wp_get_attachment_image_src( get_post_thumbnail_id( $item_id ), $items_media_size );
					$item                             = array();
					...
					$item['item_comments_number']     = $items_commentscount && get_comments_number() > 0 ? get_comments_number() : 0;
					$item['item_comments_link']       = $items_commentscount && get_comments_number() > 0 ? get_comments_link() : '';
					$item['item_comments_text']       = $items_commentscount && get_comments_number() > 0 ? esc_html__( 'comments', 'plethora-framework' ) : '';
					...
					// add it to $items
					$items[] = apply_filters( 'plethora_anypostloop_'. $this->post_type->name .'_'. $this->sc_type .'_item_opts', $item, $this->post_type, $this->sc_type );
		...
		public function prepare_atts_for_main_template_query_args( $atts ) {
			...
			if ( $data !== $this->post_type->name .'_selection' ) { // if not a custom selection
				...
				foreach ( $data_tax_exclude as $tax_term ) { // exclude posts with terms
					...
					$tax_term            = explode('|', $tax_term );
					$args['tax_query'][] = array( 'taxonomy' => trim( $tax_term[0] ), 'field' => 'term_id', 'terms' => $tax_term[1], 'operator' => 'NOT IN' );
				}
		...
		*/
		public function prepare_atts_for_filterbar_template( $atts ) {
			...
			if ( !empty($atts['filterbar_tax_exclude']) ){

				foreach (explode(',', $atts['filterbar_tax_exclude'] ) as $value) {

					$filter_tax_term    = explode('|', $value );
					$filter_tax_term    = trim($filter_tax_term[1]);
					$exclude_term_ids[] = $filter_tax_term;
...
(function($){
	...
	$(window).load(function(){
		...
		var slidesLength = $slider.find(".' . $slider["post_type"] . 's_list_item").length;
		var loop         = ( slidesLength > 1 ) ? _p.checkBool('.  $slider["loop"] .') : false;
		...
		$slider.owlCarousel({
			...
			navText            : ["'. $slider["nav_text_prev"] .'", "'. $slider["nav_text_next"] .'"],
